...
function Orders() {
 ...
 const fetchChefs = async () => {
  ...
  const assignTable = async (orderId, tableId) => {
    try {
       ...
       if (!table) return;
           ...
           await axios.put(`http://localhost:8000/api/orders/${orderId}/assign-table`, {
        tableNumber: table.tableNumber,
       });

     await axios.patch(`http://localhost:8000/api/tables/${tableId}/status`, { status: 'Reserved' });
  ...
  const assignChef = async (orderId, chefId) => {
    try {
      await axios.put(`http://localhost:8000/api/orders/${orderId}/assign-chef`, { chefId });
      fetchOrders();
      fetchChefs();
    } catch (err) {
